...
時点 $t$ の初めに技術ショックが実現し、家計はそれを観察してから消費、労働、債券、企業持分を選びます。期首に持ち越す $B_{t-1}(h)$ と $\Theta_{t-1}(h)$ は所与です。同じ選好と初期保有 $B_{-1}(h)=0$、$\Theta_{-1}(h)=1$ を持つ家計が、同じ行動を選ぶ対称均衡を考えます。
...
$$
左辺は消費と資産の購入、右辺は前期から保有する資産の収益と労働所得です。$R_{t-1}$ は前期に購入した実質債券の今期総収益率です。$V_t$ は配当落ち株価なので、前期から株式を保有する家計は $V_t+D_t$ を受け取ります。
...
$A_t^F(h)$ は市場価格で評価した期末の純金融資産であり、$\bar L$ は所与の信用枠です。

以下では、この信用限度が拘束せず、消費・労働の選択が内点となる対称均衡を扱います。また、期待効用や資産価格が適切に定まると仮定します。

第3回と同じく、対称均衡の確率的割引因子（SDF）を
$$
...
\end{aligned}
$$ {#eq-household-focs}
第1式は労働供給条件で、追加的に働く負担と賃金による便益を等しくします。第2式は実質債券のオイラー方程式で、現在と将来の消費を結びます。第3式は、第3回で導いた株式の価格付け式です。

さらに、第3回と同じく、期末の純金融資産に対して横断性条件
$$
\lim_{N\to\infty}
\mathbb E_t\left[
Q_{t,t+N}\bigl(B_{t+N}+V_{t+N}\Theta_{t+N}\bigr)
\right]=0
$$ {#eq-household-tvc}
を課します。ここで $Q_{t,t+N}=\beta^NU_{C,t+N}/U_{C,t}$ は多期間のSDFです。第3回の債券の市場価値を、この回では債券と株式を合わせた市場価値に置き換えています。この条件は、将来資産のSDF割引現在価値が無限遠でゼロになることを表します。最適性の厳密な証明は、本講義では扱いません。

::: {.callout-note title="第3回との接続：株式価値"}
第3回の配当割引モデルと同じく、無バブル条件のもとで、株式価値は将来配当の割引現在価値です。
$$
V_t=\sum_{i=1}^{\infty}\mathbb E_t[Q_{t,t+i}D_{t+i}].
$$ {#eq-fundamental-equity-value}
ここで $Q_{t,t+i}=\beta^iU_{C,t+i}/U_{C,t}$ は多期間のSDFです。本講義では、この無バブル価格を用います。
:::
...
\int\Theta_t(h)\,dh=1
$$
家計の予算制約を集計すると、純供給ゼロの債券は消え、株式の売買代金は左右で相殺されます。したがって、
$$
C_t=D_t+W_tN_t
$$
です。企業利潤 $D_t=Y_t-W_tN_t$ を代入すると、財市場の均衡 $Y_t=C_t$ が得られます。

対称均衡では、各家計の保有も $B_t=0$、$\Theta_t=1$ です。非負の配当を現在価値で評価した株価も非負なので、家計は借入限度に達していません。
...
- Galí, Jordi (2015) *Monetary Policy, Inflation, and the Business Cycle* (2nd ed.). Princeton University Press. 第2章が、資本蓄積のない柔軟価格モデルと自然実質利子率の参照先。[著者の第2章スライド](https://crei.cat/wp-content/uploads/users/pages/ch2_slides_october_2015.pdf)。
- Romer, David (2019) [*Advanced Macroeconomics* (5th ed.)](https://www.mheducation.com/highered/product/advanced-macroeconomics-romer.html?viewOption=student). McGraw-Hill. 実物ショックと標準的な資本蓄積モデルを比較するための参照先。
...
- King, Robert G., and Sergio T. Rebelo (1999) "Resuscitating Real Business Cycles." In John B. Taylor and Michael Woodford (eds.), *Handbook of Macroeconomics*, Vol. 1B, ch. 14, pp. 927-1007. Elsevier. [doi](https://doi.org/10.1016/S1574-0048(99)10022-3)
