...
$$c_t = E_t c_{t+1} + \sigma E_t q_{t,t+1}^j.$$
...
$$
c_t^j = -\delta\beta \sum_{i=0}^{\infty} \beta^i E_t r_{t+i} + (1-\beta) \sum_{i=0}^{\infty} \beta^i E_t y_{t+i}.
$$
In recursive form, this is given by:
$$
c_t^j = (1 - \beta)y_t^j - \delta\beta r_t + \beta E_t c_{t+1}^j.
$$
...
We can also analyse the effect of government spending on output and consumption using the Keynesian-cross representation of the RANK model. Suppose that the government introduces a one-time increase in government spending, which is financed by taxation: $g^t = t^t$ .

Replacing output $y^t$ with disposable income $\hat{y}^t - t^t$, the PE curve is equal to:
$$
...
\pi_t = \kappa c_t
$$
where κ is the elasticity of inflation to changes in consumption. Substituting this into the Euler-IS equation gives:
$$
...
The intuition behind this result can be shown within a very simple RANKstyle framework. Suppose that output is produced using a constant returns to scale production technology:
$$
Y_t = A_t L_t
$$
