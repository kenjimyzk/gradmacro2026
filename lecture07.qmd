...
大文字は水準、小文字は定常状態からの対数乖離を表します。第7回では $\alpha=0$、政府支出なし、$a=0$、$\nu=1$ と置き、技術水準 $\exp(a)=1$ の定常状態で $N=Y=C=W=MC=1$ となるように正規化します。
...
| $\kappa_x$ | $\kappa_p(\gamma+\varphi)$ | 需給ギャップに対するインフレの反応係数 |
...
文献では、Calvo (1983) 型価格設定もよく使われます。Calvo 型では、各期に一定割合の企業だけが価格を改定できると仮定します。価格硬直性の入れ方は異なりますが、ゼロインフレ定常状態の周りの一次近似では、Rotemberg 型も Calvo 型も同じ形の NKフィリップス曲線
$$
\pi_t=\beta\mathbb{E}_t\pi_{t+1}+\kappa_p mc_t
$$
を導きます。この講義では、企業の価格設定条件を価格調整費用として明示できる Rotemberg 型を使います。
...
$$
とします。ここでは $\nu=1$ と正規化しています。この正規化により、$a=0$、$\alpha=0$ の定常状態で $N=Y=C=W=1$ になります。
...
ゼロインフレ定常状態を考えます。技術の定常状態を $a=0$、つまり技術水準を $\exp(a)=1$ とし、インフレ率を
$$
\Pi=1
$$
...
ゼロインフレ定常状態 $\Pi=1$ の周りで対数線形化します。第7回では $Y=C=N=W=MC=1$ かつ $a=0$ なので、小文字は定常状態からの対数乖離です。対数線形化された条件を網羅的に書くと、次の体系になります。
