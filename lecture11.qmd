...
ここで $\tau_D$ は、集計純配当の変動のうち Type H へ再分配する割合を表す一次近似上の負荷係数です。$\Theta_D$ はそのルールのもとで閉じた需要曲線の傾きを決める係数、$\chi$ は所得フィードバックの合成パラメータです。この $\chi$ は、第10回の Markov 裁量問題で継続価値の微分を表した $\chi_t$ とは別の記号です。再分配がない $\tau_D=0$ では
$$
\chi=1+\varphi,
\qquad
...
-\frac{1}{\gamma\Theta_T}(r_t-r_t^{f,T})
$$
と書けることを示しなさい。$r_t^{f,T}$ が柔軟価格の TANK 配分を支える利子率である理由も説明しなさい。
...
$$
と定義する。$\tau_D=0$ なら $\chi=1+\varphi$、$\Theta_D=\Theta_T$ となることを示しなさい。$\tau_D$ が変わると NKクロスの所得フィードバックと需要曲線の傾きがどう変わるか説明しなさい。なお、ここでの $\chi$ は第10回の継続価値項 $\chi_t$ とは別物である。
