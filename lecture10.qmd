...
**問5 [Research extension] [Derivation]：Markov 裁量政策**
...
$$
と置きます。$mc_t$ の一階条件に $\Gamma_t^{mc}$ が入る理由を説明し、Markov 裁量政策のターゲット条件
$$
...
\right]
+
\kappa_w(\gamma+\varphi)\Gamma_t^{mc}
=0
$$
を導出しなさい。また、一期間問題または終端期には $\Gamma_t^{mc}=0$ となる理由を説明しなさい。
...
**問7 [Research extension] [Derivation]：柔軟賃金極限**

$\eta_w\to0$ のとき、$\omega_w\to0$、$\kappa_w\to\infty$、$\Gamma_t^{mc}\to0$ となることを使い、問5の条件が第9回の価格硬直性だけのターゲット条件へ戻ることを示しなさい。
...
=0.
$$
です。$\Gamma_t^{mc}$ は、当期の実質賃金を変えることで翌期以降の調整費用と安定化余地がどう変わるかを表します。一般には価値関数を解かなければ $\Gamma_t^{mc}$ は決まらないため、Markov 裁量解は当期変数だけの静学条件には縮約できません。

一期間だけを最適化する場合、または有限期間問題の終端期には $\Gamma_t^{mc}=0$ です。この条件付き静学ベンチマークでは
$$
x_t
=
-
...
この補論では、前の政策補論で示した Markov 裁量政策の一般化されたターゲット条件を導きます。$mc_t$ が翌期の状態になる点を明示し、継続価値の影 $\Gamma_t^{mc}$ を一階条件に残します。
...
将来期待は所与として微分しますが、$mc_t$ の一階条件には継続価値の限界効果 $\Gamma_t^{mc}$ が残ります。一階条件は
$$
\begin{aligned}
...
&\qquad
\kappa_p\Lambda_{1t}-\kappa_w\Lambda_{2t}+\Lambda_{3t}+\Gamma_t^{mc}=0,
\\
...
$$
です。$\Gamma_t^{mc}$ は、翌期の賃金インフレ恒等式における状態制約の影を表します。この条件と民間部門の期待形成、価値関数を同時に解くことで Markov 裁量均衡が決まります。

一期間問題または有限期間問題の終端期には継続価値がないため、$\Gamma_t^{mc}=0$ です。この場合に限り、ターゲット条件は
$$
\left(1+\kappa_p+\kappa_w\right)x_t
+
\kappa_w(\gamma+\varphi)
