...
発展補論では、この二重硬直性を第9回の政策問題へ応用します。効率的実質賃金を動かすショックのもとで神々の配剤が崩れる理由と、無限期間の Markov 裁量政策で継続価値の影が必要になる理由を確認します。この政策分析は第10回の本編を理解するための必須項目ではありません。
...
この補論では、前の政策補論で示した Markov 裁量政策の一般化されたターゲット条件を導きます。$mc_t$ が翌期の状態になる点を明示し、継続価値の影 $\chi_t$ を一階条件に残します。
