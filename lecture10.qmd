...
この回は、第7回のモデル構築と第9回の政策評価をつなぐ回です。第7回で導いた価格硬直性の RANKモデルに賃金硬直性を加え、第9回で扱った神々の配剤（divine coincidence：価格インフレ率と需給ギャップを同時に安定化できる性質）と裁量政策のターゲット条件がどう変わるかを確認します。
...
$$
第1式は2つのマークアップと需給ギャップを結び、第2式は価格・賃金インフレ率の差が実質賃金を動かすことを示します。以下では、労働パッカーと賃金設定からこの2式を導き、神々の配剤と金融政策への含意を調べます。
...
\right]
$$
です。ここでは有効下限制約を捨象し、中央銀行は自然利子率を観察して必要な実質利子率を実装できるとします。

第9回との違いは、実質限界費用 $mc_t$ が翌期の状態になることです。賃金インフレ恒等式の1期先を見れば、当期の $mc_t$ は翌期の $mc_{t-1}$ として残ります。したがって、裁量政策でも中央銀行は、当期の実質賃金が将来厚生へ与える効果を無視できません。

$s_t\equiv mc_{t-1}$ を期首状態、$\xi_t$ を技術などの外生状態とします。Markov 裁量政策の価値関数は概念的に
$$
V(s_t,\xi_t)
=
\min_{\pi_t^p,\pi_t^w,x_t,mc_t,r_t}
\left\{
\frac{1}{2}
\left[
x_t^2
...
+\omega_w(\pi_t^w)^2
\right]
+\beta\mathbb{E}_tV(mc_t,\xi_{t+1})
\right\}
$$
と書けます。制約は上の価格・賃金フィリップス曲線、賃金インフレ恒等式、IS曲線です。将来の民間期待を当期の一階条件で所与としても、継続価値は残ります。実質限界費用を翌期へ持ち越す限界的な価値を
$$
\chi_t
\equiv
\beta\mathbb{E}_t
V_s(mc_t,\xi_{t+1})
$$
と置くと、Markov 裁量政策が満たす一般化されたターゲット条件は
$$
...
\right]
+
\kappa_w(\gamma+\varphi)\chi_t
=0.
$$
です。$\chi_t$ は、当期の実質賃金を変えることで翌期以降の調整費用と安定化余地がどう変わるかを表します。一般には価値関数を解かなければ $\chi_t$ は決まらないため、Markov 裁量解は当期変数だけの静学条件には縮約できません。

一期間だけを最適化する場合、または有限期間問題の終端期には $\chi_t=0$ です。この条件付き静学ベンチマークでは
$$
x_t
=
-
...
\right].
$$
を得ます。この式は政策金利の反応関数ではなく、条件付き静学問題で選ばれる配分のターゲット条件です。$\omega_w$ が大きいほど、同じ賃金インフレ率に対してこの条件が要求する負の産出ギャップが大きくなります。

選ばれた配分を実装する実質利子率と名目利子率は、IS曲線とフィッシャー関係から
$$
\begin{aligned}
r_t
&=
...
\mathbb{E}_t x_{t+1}-x_t
\right)
+\mathbb{E}_t\pi_{t+1}^p
\end{aligned}
$$
と求めます。ターゲット条件は望ましい配分を記述し、この2式はその配分を実装する政策手段を記述します。

柔軟賃金への移行は、賃金調整費用を $\eta_w\to0$ とする1つの構造的な極限です。このとき
$$
\omega_w=\frac{\eta_w}{\gamma+\varphi}\to0,
\qquad
\kappa_w=\frac{\psi_w}{\eta_w}\to\infty,
\qquad
\chi_t\to0
$$
この極限では実質賃金の持越し状態も消えます。一般化された条件は
$$
x_t
=
-
...
$$
に近づきます。これは、第9回の価格硬直性だけの裁量政策条件と同じ形です。ここで $\kappa_p(\gamma+\varphi)$ は、価格硬直性だけの NKフィリップス曲線の傾きです。
...
政策面では、効率的実質賃金を動かすショックのもとで神々の配剤が崩れます。さらに $mc_t$ は翌期の状態になるため、無限期間の Markov 裁量政策のターゲット条件には継続価値の影 $\chi_t$ が入ります。$\chi_t=0$ の式は一期間または終端期のベンチマークであり、一般の動学的裁量解ではありません。ターゲット条件は望ましい配分を記述し、政策金利は IS曲線とフィッシャー関係から実装します。
...
- Erceg, Christopher J., Dale W. Henderson, and Andrew T. Levin (2000) "Optimal Monetary Policy with Staggered Wage and Price Contracts." *Journal of Monetary Economics*, 46(2), 281-313. [doi:10.1016/S0304-3932(00)00028-3](https://doi.org/10.1016/S0304-3932(00)00028-3). 価格・賃金硬直性のもとで生じる3変数間の政策トレードオフの基礎文献。
- Rotemberg, Julio J. (1982) "Sticky Prices in the United States." *Journal of Political Economy*, 90(6), 1187-1211. [doi:10.1086/261117](https://doi.org/10.1086/261117). 本講義で使う調整費用型の名目硬直性の原典。
...
- Christiano, Lawrence J., Martin Eichenbaum, and Charles L. Evans (2005) "Nominal Rigidities and the Dynamic Effects of a Shock to Monetary Policy." *Journal of Political Economy*, 113(1), 1-45. [doi:10.1086/426038](https://doi.org/10.1086/426038)
- Smets, Frank, and Raf Wouters (2003) "An Estimated Dynamic Stochastic General Equilibrium Model of the Euro Area." *Journal of the European Economic Association*, 1(5), 1123-1175. [doi:10.1162/154247603770383415](https://doi.org/10.1162/154247603770383415)
- Calvo, Guillermo A. (1983) "Staggered Prices in a Utility-Maximizing Framework." *Journal of Monetary Economics*, 12(3), 383-398. [doi:10.1016/0304-3932(83)90060-0](https://doi.org/10.1016/0304-3932(83)90060-0)
...
賃金インフレ恒等式により、今日の $mc_t$ は翌期の状態になります。継続価値の限界効果を
$$
\chi_t
\equiv
\beta\mathbb{E}_tV_s(mc_t,\xi_{t+1})
$$
と置きます。$mc_t$ の一階条件に $\chi_t$ が入る理由を説明し、Markov 裁量政策のターゲット条件
$$
\left(1+\kappa_p+\kappa_w\right)x_t
+
\kappa_w(\gamma+\varphi)
...
=0
$$
を導出しなさい。また、一期間問題または終端期には $\chi_t=0$ となる理由を説明しなさい。

**問6：ターゲット条件と政策金利**

問5の式が望ましい配分を表すターゲット条件であり、政策金利の反応関数ではないことを説明しなさい。さらに、選ばれた配分を実現する
$$
r_t
=
r_t^f+\gamma(\mathbb{E}_t x_{t+1}-x_t),
\qquad
r_t^N
=
r_t+\mathbb{E}_t\pi_{t+1}^p
$$
を IS曲線とフィッシャー関係から導出しなさい。

**問7：柔軟賃金極限**

$\eta_w\to0$ のとき、$\omega_w\to0$、$\kappa_w\to\infty$、$\chi_t\to0$ となることを使い、問5の条件が第9回の価格硬直性だけのターゲット条件へ戻ることを示しなさい。
...
# 補論（発展）：Markov 裁量政策のターゲット条件

この補論では、本文で使った Markov 裁量政策の一般化されたターゲット条件を導きます。$mc_t$ が翌期の状態になる点を明示し、継続価値の影 $\chi_t$ を一階条件に残します。
...
s_t\equiv mc_{t-1}
$$
と置きます。当期に選ぶ $mc_t$ は翌期の $s_{t+1}$ になるため、裁量政策でも内生状態です。外生状態をまとめて $\xi_t$ と書くと、価値関数は
$$
V(s_t,\xi_t)
=
\min_{\pi_t^p,\pi_t^w,x_t,mc_t,r_t}
...
裁量政策の一時的な逸脱を考えるとき、民間部門が形成した将来期待は所与として扱います。ただし、$mc_t$ が翌期の状態になることによる継続価値への効果は残ります。この効果を
$$
\chi_t
\equiv
\beta\mathbb{E}_t
...
\end{aligned}
$$
ここで $r_t$ は政策手段として選べる実質利子率です。名目利子率はフィッシャー方程式を通じて実装されます。
...
将来期待は所与として微分しますが、$mc_t$ の一階条件には継続価値の限界効果 $\chi_t$ が残ります。一階条件は
$$
\begin{aligned}
...
&\qquad
\kappa_p\Lambda_{1t}-\kappa_w\Lambda_{2t}+\Lambda_{3t}+\chi_t=0,
\\
...
\Lambda_{4t}=0
$$
したがって、IS曲線はターゲット条件を決める制約ではなく、選んだ配分を実装する政策金利を決める条件になります。
...
これを $x_t$ について解けば、本文の一般化されたターゲット条件
$$
x_t
=
-
...
価値関数の包絡条件は
$$
V_s(s_t,\xi_t)=-\Lambda_{3t}
$$
なので、
$$
\chi_t
=
-\beta\mathbb{E}_t\Lambda_{3,t+1}
$$
です。$\chi_t$ は、翌期の賃金インフレ恒等式における状態制約の影を表します。この条件と民間部門の期待形成、価値関数を同時に解くことで Markov 裁量均衡が決まります。

一期間問題または有限期間問題の終端期には継続価値がないため、$\chi_t=0$ です。この場合に限り、ターゲット条件は
$$
\left(1+\kappa_p+\kappa_w\right)x_t
+
\kappa_w(\gamma+\varphi)
...
=0
$$
この式は当期変数だけで構成されます。無限期間の Markov 裁量条件ではなく、継続価値を持たない条件付き静学ベンチマークです。
...
\chi_t\to0
$$
を同時に取ります。一般化された条件を $\kappa_w$ で割って極限を取ると、
$$
x_t
+
\kappa_p(\gamma+\varphi)\omega_p\pi_t^p
