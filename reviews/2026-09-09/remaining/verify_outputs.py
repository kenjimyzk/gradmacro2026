#!/usr/bin/env python3
"""Static checks for the six final rendered documents; no browser is used."""
...
ROOT = OUT.parents[2]
NAMES = ['lecture01', 'lecture03', 'lecture05', 'lecture07', 'lecture09', 'solution']
...
for name in NAMES:
    ...
    checks = {
        ...
        'html_sha256': sha(h),
        'browser_layout': 'not verified: local file navigation was blocked by browser security policy',
    }
    ...
    sources[name] = {
        'display_delimiters_even': len(re.findall(r'^\$\$\s*$', q, re.M)) % 2 == 0,
...
print('Verified static HTML references/assets, PDF text, and display-math delimiters for six documents.')
