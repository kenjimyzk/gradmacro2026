...
| $\kappa_x$ | $MC(\tilde{\gamma}+\tilde{\varphi})\psi/\eta$ | 需給ギャップに対するインフレの反応係数 |
| $\kappa_i,\; i\in\{a,m,g\}$ | $\kappa_x/(1-\beta\rho_i)$ | ショック $i$ 用のフィリップス曲線係数 |
...
\pi_t=\kappa_i x_t,
\qquad
\kappa_i=\frac{\kappa_x}{1-\beta\rho_i}
$$
...
\pi_t
=
\beta\rho_a\pi_t+\kappa_x x_t
$$
なので、
$$
\pi_t=\kappa_a x_t,
\qquad
\kappa_a\equiv\frac{\kappa_x}{1-\beta\rho_a}
$$
です。IS 曲線は
$$
x_t
...
です。ここで
$$
\kappa_a=\frac{\kappa_x}{1-\beta\rho_a},
\qquad
\Omega_a
=
...
\end{aligned}
$$
です。なお、価格が柔軟なとき（$\kappa_x\to\infty$）、$\Omega,\;\Omega_{m},\;\Omega_{a},\;\Omega_{g}\to 0$なので、$x_t=0$。
価格が一定のとき（$\kappa_x\to0$）、$\Omega\to1/\{\tilde{\gamma}(1-\rho_{m})\}, \Omega_{m},\Omega_{a},\Omega_{g}\to1$なので、$x_{t}=m_t/{\tilde{\gamma}(1-\rho_m)}-\zeta_{a}a_{t}+(1-\zeta_{g})g_{t}$となります。
