...
@fig-laffer-curve は、$\alpha=0.4$、$\gamma=\varphi=1$、$\mathcal A=1$ のもとで $TX(\tau_N)/\mathcal A=\tau_N(1-\tau_N)^\vartheta$ を描いています。HTML版では作図コードを展開できます。
...
@eq-government-debt-law を前向きに反復し、@eq-government-transversality のもとで @eq-government-intertemporal-budget を導出しなさい。現在の政府債務を裏づけるプライマリーバランスは、なぜ $t+1$ 期以降のものになるかも説明しなさい。
